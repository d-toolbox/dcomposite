...
(function() {
  var appendMessage, checkForm, fitSimToScreen, getSimulateURL, hideSimulator, initSimulatorBody, initTextareaHeight, level_classes, moving_sim, processForm, resized, sendAudio, sendGPS, sendMessage, sendPhoto, sendVideo, showSimulator, toExpand, verifyNumberSimulator;
  ...
  window.updateSimulator = function(data) {
    var activity, direction, ele, i, j, len, level, media_type, model, msg, node, ref, scope, top, ussd;
    ussd = window.ussd ? "ussd" : "";
    $(".simulator-body").html("");
    i = 0;
  ...
  appendMessage = function(newMessage, ussd) {
    var imsgDiv;
    if (ussd == null) {
      ussd = false;
    }
  ...
  $("#simulator .send-message").on("click", function() {
    ...
    if (window.ussd && newMessage.length <= 182) {
      return appendMessage(newMessage, true);
    } else if (newMessage.length <= 160) {
      return appendMessage(newMessage);
    }
  });
