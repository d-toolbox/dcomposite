...
(function() {
  ...
  app.directive("ussd", [
    "$rootScope", "$log", "Flow", "utils", function($rootScope, $log, Flow, utils) {
      var MESSAGE_LENGTH, link;
      MESSAGE_LENGTH = 182;
      link = function(scope, element, attrs) {
        var base, base1, base2, insertEmpty, menu, name, refreshMenu, updateCategory;
        ...
        (insertEmpty = function() {
          ...
          if (scope.USSD_MENU) {
            if (scope.ruleset.rules.filter(function(rule) {
              return !rule.label && rule._config.show;
            })) {
              scope.ruleset.rules = scope.ruleset.rules.filter(function(rule) {
                return rule.label || (!rule.label && !rule._config.show);
              });
              refreshMenu();
            }
      ...
      link = function(scope, element, attrs, form) {
        ...
        element.select2({
          ...
          query: function(query) {
            ...
            if (query.term && data.results.length === 0 && query.term.strip().length > 0 && query.term.strip().length <= 36 && /^[a-zA-Z0-9-][a-zA-Z0-9- ]*$/.test(query.term.strip())) {
              data.results.push({
                id: '[_NEW_]' + query.term,
